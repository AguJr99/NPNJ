      ...
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        <div className="mb-4">
          ...
          <h3 
            ...
            className="text-sm md:text-xl font-sans font-black text-secondary leading-tight group-hover:text-primary transition-colors uppercase tracking-tight cursor-pointer min-h-[2.5rem] md:min-h-[3rem] flex items-center"
          >
            {jersey.team}
          </h3>
          <div className="mt-2 flex flex-row gap-1 md:gap-2">
            {(jersey.isRetro ? ['Retro', 'Niño'] : ['Fan', 'Player', 'Niño']).map(v => (
              <span key={v} className="px-1 py-0.5 md:px-1.5 md:py-1 bg-secondary/5 rounded-md text-[6px] md:text-[8px] font-black text-secondary/40 uppercase tracking-widest border border-secondary/5 whitespace-nowrap">
                {v}
              </span>
            ))}
          </div>
        ...
        <button
          onClick={() => onOrder(jersey)}
          className="w-full bg-secondary text-primary py-2.5 md:py-4 rounded-xl md:rounded-2xl font-black text-[8px] md:text-[10px] uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-primary hover:text-secondary transition-all duration-300 shadow-xl shadow-secondary/10 active:scale-95 flex items-center justify-center gap-1.5 md:gap-2"
        >
          <ClipboardList className="w-3.5 h-3.5 md:w-4 md:h-4 ml-0.5" />
...
const EncargoOrderModal = ({ jersey, onClose, onZoom }: { jersey: EncargoJersey, onClose: () => void, onZoom: (src: string) => void }) => {
  ...
  const getPatches = () => {
    ...
    if (jersey.team === 'Atlético de Madrid') {
      return [
        { name: 'Sin Parche', logo: null },
        { name: 'Champions League', logo: 'https://drive.google.com/thumbnail?id=1yDgD4gDXDtoB7o2QDNoHFEWrhVmr9nS2&sz=w200' },
        ...
        <div className="flex-grow overflow-y-auto bg-accent scrollbar-hide">
          <div className="max-w-xl mx-auto p-6 md:p-10 space-y-10 pb-24">
            ...
            <section>
              ...
              <div className={`grid ${jersey.isRetro ? 'grid-cols-2' : 'grid-cols-3'} gap-3`}>
                {(jersey.isRetro ? ['Retro', 'Niño'] : ['Fan', 'Player', 'Niño']).map(v => (
                  <button
                    key={v}
                    onClick={() => {
                ...
                <div className="space-y-4 w-full">
                  ...
                  <div className="relative cursor-zoom-in group shadow-lg rounded-2xl overflow-hidden max-w-[400px] mx-auto border border-secondary/5 bg-white h-auto" onClick={() => onZoom(currentNumbering)}>
                    ...
                    <img 
                      ...
                      alt="Dorsal" 
                      className={`w-full h-auto max-h-[250px] md:max-h-[350px] object-contain transition-opacity duration-300 ${imagesLoaded.numbering ? 'opacity-100' : 'opacity-0'}`}
...
export default function App() {
  ...
  const filteredEncargoJerseys = ENCARGO_JERSEYS.filter(j => {
    const matchesSearch = searchQuery ? (j.team.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         j.name.toLowerCase().includes(searchQuery.toLowerCase())) : true;
    const matchesTeam = !selectedTeam || j.team === selectedTeam;
          ...
          <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
            <div className={`flex flex-col items-center ${activeTab === 'encargos' ? 'gap-6 md:gap-8 mb-10' : 'gap-12 mb-16'}`}>
              ...
              <div className={`w-full flex flex-col items-center ${activeTab === 'encargos' ? 'gap-6 md:gap-8' : 'gap-8 md:gap-12'}`}>
                ...
                {activeTab === 'encargos' && !searchQuery && (
            ...
            {activeTab === 'stock' ? (
              ...
              <div className="space-y-12">
                {(searchQuery || selectedTeam) && filteredEncargoJerseys.length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                {(searchQuery || selectedTeam || selectedLeague) && (
                  <div className="text-center py-6 md:py-10 bg-white rounded-3xl md:rounded-[2rem] border border-secondary/5 shadow-sm px-6 max-w-3xl mx-auto">
                    ...
                    >
                      ...
                      <button 
                        onClick={() => {
                          const query = searchQuery || selectedTeam || '';
                          const message = query 
