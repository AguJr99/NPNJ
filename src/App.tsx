...
const JerseyCard = ({ jersey, onOrder }: { jersey: Jersey, onOrder: (j?: Jersey) => void, key?: string }) => {
  ...
  return (
    <>
      ...
      >
        <div 
          className="relative aspect-square overflow-hidden bg-accent/30 cursor-zoom-in"
        ...
        <div className="p-4 md:p-8 space-y-4 md:space-y-7 flex-grow flex flex-col items-center text-center">
          ...
          <div className="w-full grid grid-cols-2 gap-px bg-secondary/5 rounded-xl md:rounded-2xl overflow-hidden border border-secondary/5">
            <div className="bg-white p-2 md:p-3.5 space-y-0.5 md:space-y-1">
              <p className="text-[6px] md:text-[8px] font-black text-secondary/60 uppercase tracking-[0.25em]">Estilo</p>
          ...
          <div className="pt-4 md:pt-5 mt-auto border-t border-secondary/5 w-full space-y-4 md:space-y-5">
            ...
            <button
              onClick={() => {
                const message = `¡Hola! Me interesa la camiseta de ${jersey.team} (${jersey.season}) - Talla ${jersey.size} - Estilo ${jersey.style} - ${jersey.playerName ? `Nombre: ${jersey.playerName} #${jersey.number}` : 'Sin personalización'}.`;
...
const EncargoJerseyCard = ({ jersey, onOrder }: { jersey: EncargoJersey, onOrder: (j: EncargoJersey) => void, key?: string }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-secondary/5 group border border-secondary/5 flex flex-col hover:shadow-primary/10 transition-all duration-500"
    >
      <div className="relative aspect-square overflow-hidden bg-accent/30 cursor-pointer" onClick={() => onOrder(jersey)}>
        {!isLoaded && (
          <div className="absolute inset-0 bg-secondary/5 animate-pulse flex items-center justify-center">
            <Shirt className="w-8 h-8 text-secondary/10" />
          </div>
      ...
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] font-black text-primary uppercase tracking-widest">{jersey.type}</span>
            <span className="w-1 h-1 rounded-full bg-secondary/20"></span>
            <span className="text-[10px] font-black text-secondary/40 uppercase tracking-widest">{jersey.season}</span>
          </div>
          <h3 
            onClick={() => onOrder(jersey)}
            className="text-lg md:text-xl font-sans font-black text-secondary leading-tight group-hover:text-primary transition-colors uppercase tracking-tight cursor-pointer"
          >
            {jersey.name}
          </h3>
          <div className="mt-3 flex flex-wrap gap-2">
            {['Fan', 'Player', 'Niño'].map(v => (
              <span key={v} className="px-2 py-1 bg-secondary/5 rounded-md text-[8px] font-black text-secondary/40 uppercase tracking-widest border border-secondary/5">
                {v}
              </span>
            ))}
          </div>
        ...
        <button
          onClick={() => onOrder(jersey)}
          className="w-full bg-secondary text-primary py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-primary hover:text-secondary transition-all duration-300 shadow-xl shadow-secondary/10 active:scale-95 flex items-center justify-center gap-2"
        >
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-secondary/80 backdrop-blur-sm">
      ...
      >
        <div className="p-6">
          ...
          <button
            onClick={handleSend}
            className="w-full mt-8 bg-primary text-secondary py-4 rounded-2xl font-sans font-bold text-lg hover:bg-primary/80 transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
          >
...
const EncargoOrderModal = ({ jersey, onClose, onZoom }: { jersey: EncargoJersey, onClose: () => void, onZoom: (src: string) => void }) => {
  ...
  const basePrice = form.version === 'Player' ? 35 : 30;
  const totalPrice = form.sleeves === 'Larga' ? basePrice + 3 : basePrice;

  const isBarcelonaCuarta = jersey.id === 'fcb-fourth-25-custom';

  let currentImage = form.version === 'Fan' ? jersey.fanImage : form.version === 'Player' ? jersey.playerImage : jersey.childImage;
  if (form.sleeves === 'Larga' && !isBarcelonaCuarta) {
    if (form.version === 'Fan' && jersey.fanLongSleeveImage) currentImage = jersey.fanLongSleeveImage;
    if (form.version === 'Player' && jersey.playerLongSleeveImage) currentImage = jersey.playerLongSleeveImage;
  }

  const currentSizeGuide = form.version === 'Fan' ? jersey.fanSizeGuide : form.version === 'Player' ? jersey.playerSizeGuide : jersey.childSizeGuide;
  const currentNumbering = (form.patch === 'Champions League' || form.patch.includes('Champions')) ? jersey.championsNumberingImage : jersey.ligaNumberingImage;
...
Talla: *${form.size}*
Mangas: *${isBarcelonaCuarta ? 'Corta' : form.sleeves}*
Nombre: *${form.name || 'Sin nombre'}*
Dorsal: *${form.number || 'Sin número'}*
Parche: *${form.patch}*
Precio: *$${totalPrice}*`;
        ...
        <div className="bg-white border-b border-secondary/5 p-4 md:p-6 flex items-center justify-between z-50 shadow-sm shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-primary font-black text-[10px] md:text-xs uppercase tracking-widest">{jersey.name}</p>
            </div>
          </div>
          ...
          <div className="max-w-xl mx-auto p-6 md:p-10 space-y-10 pb-24">
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-3">
                {(form.version === 'Niño' ? ['16', '18', '20', '22', '24', '26', '28'] : ['S', 'M', 'L', 'XL', 'XXL']).map(s => (
                  <button
                    key={s}
                    onClick={() => setForm({ ...form, size: s })}
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-4">
                {PATCHES.map(p => (
                  ...
                  >
                    {p.logo ? (
                      <img src={p.logo} alt={p.name} className="w-16 h-16 md:w-20 md:h-20 object-contain" referrerPolicy="no-referrer" />
                    ) : (
                      <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center border-2 border-dashed border-secondary/10 rounded-full">
            ...
            <section className="h-[250px] md:h-[350px] flex items-center justify-center pb-10">
              {currentNumbering && form.patch !== 'Sin Parche' ? (
                <div className="space-y-4 w-full">
                  <label className="block text-[10px] font-black text-secondary/40 uppercase tracking-widest mb-2 text-center">
                    Estilo de Dorsal ({
                      jersey.team === 'Real Madrid' 
                        ? 'Real Madrid' 
                        : (form.patch === 'Champions League' ? 'Champions' : form.patch === 'Supercopa de España' ? 'Liga/Supercopa' : 'Liga')
                    })
                  </label>
                  <div className="relative cursor-zoom-in group shadow-lg rounded-2xl overflow-hidden max-w-[400px] mx-auto border border-secondary/5 bg-white h-auto" onClick={() => onZoom(currentNumbering)}>
                    {!imagesLoaded.numbering && <div className="h-40 w-64 bg-white animate-pulse" />}
                    <img 
...
export default function App() {
  ...
  const filteredJerseys = JERSEYS.filter(j => {
    const matchesSearch = j.team.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (j.playerName && j.playerName.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const handleStockOrder = (jersey: Jersey) => {
    const text = `¡Hola! Estoy interesado en la camiseta en stock: *${jersey.name}*. ¿La tienen disponible en talla M?`;
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(text)}`, '_blank');
  };
                  ...
                  <div className="relative flex-1 max-w-2xl w-full">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (activeTab === 'encargos') setSelectedLeague(null);
                      }}
                    ...
                    <div className="flex flex-wrap justify-center gap-3 md:gap-6">
                      {LEAGUES_DATA.map((league) => (
                        <button
                          ...
                          className={`flex flex-col items-center gap-2 md:gap-3 p-3 md:p-5 rounded-2xl border-2 transition-all duration-300 w-[90px] md:w-[130px] ${
                        ...
                        >
                          <img 
                            ...
                            referrerPolicy="no-referrer"
                          />
                          <span className="text-[9px] md:text-[11px] font-black uppercase tracking-wider text-center leading-tight">{league.name}</span>
                        </button>
                      ))}
                    </div>
                        ...
                        >
                          {LEAGUES_DATA.find(l => l.name === selectedLeague)?.teams.map((team) => (
                            <button
                              ...
                              className={`flex flex-col items-center gap-2 p-3 md:p-4 rounded-xl transition-all w-[80px] md:w-[110px] ${
                            ...
                            >
                              <img 
                                ...
                                referrerPolicy="no-referrer"
                              />
                              <span className="text-[8px] md:text-[10px] font-bold uppercase text-center leading-tight">{team.name}</span>
                            </button>
                          ))}
                        </motion.div>
                      )}
            ...
            {activeTab === 'stock' ? (
              filteredJerseys.length > 0 ? (
                ...
                <div className="text-center py-12 md:py-24 bg-white rounded-3xl md:rounded-[3rem] border border-secondary/5 shadow-inner px-6">
                  ...
                  <div className="mt-8">
                    <button 
                      onClick={() => { setSearchQuery(''); setFilterSize('TODAS'); setFilterStyle('TODOS'); }}
              ...
              <div className="space-y-12">
                {filteredEncargoJerseys.length > 0 && (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                <div className="text-center py-12 md:py-24 bg-white rounded-3xl md:rounded-[3rem] border border-secondary/5 shadow-inner px-6">
                  ...
                  >
                    <ShoppingCart className="w-12 h-12 md:w-20 md:h-20 text-primary mx-auto mb-8" />
                    <h3 className="text-xl md:text-4xl font-sans font-black text-secondary mb-6 uppercase tracking-tight">
                      {searchQuery ? `¿Quieres encargar la de ${searchQuery}?` : '¿Qué camiseta estás buscando?'}
                    </h3>
                    <p className="text-secondary/60 text-sm md:text-xl mb-10 leading-relaxed">
                      Dinos el equipo, la temporada y los detalles que prefieras. Nosotros nos encargamos de conseguirla con la mejor calidad del mercado.
                    </p>
                    <button 
                      onClick={() => {
                        const message = searchQuery 
                          ? `¡Hola! Quiero encargar la camiseta de: ${searchQuery}. ¿Me podrían dar más información?`
                          : `¡Hola! Quiero hacer un encargo personalizado. ¿Qué modelos tienen disponibles para traer?`;
                        window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`, '_blank');
                      }}
                      className="bg-secondary text-primary px-8 py-4 md:px-12 md:py-6 rounded-2xl md:rounded-[2rem] font-black text-xs md:text-lg uppercase tracking-widest hover:bg-primary hover:text-secondary transition-all shadow-2xl flex items-center gap-4 mx-auto"
                    >
