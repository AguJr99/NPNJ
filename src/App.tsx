      ...
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        <div className="flex-grow flex flex-col">
          ...
          <h3 
            ...
            className="text-lg md:text-2xl font-sans font-black text-secondary leading-none group-hover:text-primary transition-colors uppercase tracking-tighter cursor-pointer mb-4 min-h-[2.5rem] md:min-h-[3.5rem] flex items-center"
          >
            {jersey.team}
          </h3>
          <div className="mt-auto flex flex-row gap-1 md:gap-2 mb-6">
            {(jersey.isRetro ? ['Retro', 'Niño'] : ['Fan', 'Player', 'Niño']).filter(v => {
              ...
              <span key={v} className="w-10 md:w-14 py-0.5 md:py-1 bg-secondary/5 rounded-md text-[6px] md:text-[8px] font-black text-secondary/40 uppercase tracking-widest border border-secondary/5 flex items-center justify-center">
                {v}
              </span>
            ))}
          </div>
        ...
        <button
          onClick={() => onOrder(jersey)}
          className="w-full bg-secondary text-primary py-3 md:py-5 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-[0.2em] hover:bg-primary hover:text-secondary transition-all duration-300 shadow-xl shadow-secondary/10 active:scale-95 flex items-center justify-center gap-2"
        >
          <ClipboardList className="w-4 h-4 md:w-5 md:h-5 ml-0.5" />
...
const EncargoOrderModal = ({ jersey, onClose, onZoom }: { jersey: EncargoJersey, onClose: () => void, onZoom: (src: string) => void }) => {
  const getPatches = () => {
    if (jersey.team === 'Barcelona') {
      if (jersey.id === 'fcb-special-travis-24-custom') {
  ...
  const hideLongSleeves = isBarcelonaCuarta || isRmaGk || isBarcelona26 || isTravisScott || isBarcelona125;
  ...
  const currentNumbering = (form.patch === 'Champions League' || form.patch.includes('Champions') || form.patch === 'Sin Parche') 
      ...
      >
        ...
        <div className="bg-white border-b border-secondary/5 p-4 md:p-6 flex items-center justify-between z-50 shadow-sm shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {(jersey.id === 'rma-home-26-custom' || jersey.id === 'fcb-home-26-custom') && (
        ...
        <div className="flex-grow overflow-y-auto bg-accent scrollbar-hide">
          <div className="max-w-xl mx-auto p-6 md:p-10 space-y-10 pb-24">
            ...
            {jersey.id !== 'fcb-special-travis-24-custom' && (
